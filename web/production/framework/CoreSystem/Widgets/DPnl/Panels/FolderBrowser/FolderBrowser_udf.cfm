...
<cffunction name="ParseFileName" returntype="struct" output="no">
	<cfargument name="file_name" type="string" required="yes"> 

    <cfparam name="ts" default="">
    <cfset ts = StructNew()>
	
    <cfparam name="fnam" default="">
    <cfparam name="fext" default="">
    
    <cfparam name="periodPos" default="">
    <cfset periodPos = file_name.lastIndexOf(".")>
	
    <cfif periodPos EQ -1>
    	<cfset ts.PrimaryName = file_name>
        <cfset ts.SecondaryName = "">
	<cfelse>        
		<cfset fnam = Left(file_name, periodPos)>
        <cfset fext = Mid(file_name, periodPos + 2, Len(file_name))> 
        
        <cfset ts.PrimaryName = fnam>
        <cfset ts.SecondaryName = fext>
	</cfif>
    
    <cfreturn #ts#>
</cffunction>    

<cffunction name="GetParentFolder" returntype="string" output="no">
	<cfargument name="path" type="string" required="yes">
    
    <cfset path = Trim(path)>
    
    <!--- trim any trailing forward slashes --->
    <cfif Right(path, 1) EQ "/">
		
		<cfset path = Left(path, Len(path) - 1)>
	</cfif>
    
    <cfparam name="slashPos" default="">
    <cfset slashPos = path.lastIndexOf("/")>
	
    <cfparam name="t" default="">
    <cfset t = Left(path, slashPos)>
	
    <cfreturn #t#>
</cffunction>    

<cffunction name="GetFilename" returntype="string" output="no">
	<cfargument name="path" type="string" required="yes">
     ...
     <cfparam name="t" default="">
     <cfset t = Mid(path, slashPos + 1, Len(path))>
     
     <cfreturn #t#>
</cffunction>
     
<cffunction name="CheckUserFolders" returntype="void" output="yes">
