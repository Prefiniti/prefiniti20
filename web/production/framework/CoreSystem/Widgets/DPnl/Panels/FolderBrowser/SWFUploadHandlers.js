...
function fileQueued(fileObj) {
	...
	try {
		this.addFileParam(fileObj.id, "SWFUploadFileID", fileObj.id);
	} 
	catch (ex) {
		alert('Could not append file param.');
		this.debug(ex);
	}
}
...
function fileDialogComplete(num_files_queued) {
	...
	if (this.getStats().files_queued > 0) {
		...
		var wRef = p_session.Framework.FindWindow('FileOptions');
		
		if (!wRef) {
			var UChandle 	= 'FileOptions';
			var UCtitle 	= 'Uploaded File Options';
			var UCicon 		= new PIcon('/graphics/image.png', P_SMALLICON);
			var UCstyle 	= WS_ALLOWMINIMIZE | WS_ALLOWCLOSE;
			var UCrect 		= new PAutoRect(498, 496);
			
			var UCwindow = new PWindow(UChandle, 
							   UCtitle, 
							   UCrect, 
							   UCicon, 
							   UCstyle, 
			...
			wRef = p_session.Framework.CreateWindow(UCwindow);
			
			/*var ProgressDiv = 'FileUploadProgressArea';
			wRef.CreateChildFrame(ProgressDiv);*/
			
		}
		
		var url;
		url = '/contentmanager/components/cms_no_files_done.cfm?FileCount=' + escape(num_files_queued);
		
		var StatDiv = 'FUStatDiv';
		wRef.CreateChildFrame(StatDiv);
		
		
		
		AjaxLoadPageToDiv(StatDiv, url, function () { document.getElementById(this.customSettings.cancelButtonId).disabled = false; } );
	}
...
function uploadComplete(fileObj, server_data) {
	try {
		...
		var wRef = p_session.Framework.FindWindow('FileOptions');
		var URL;
		URL = '/contentmanager/components/cms_file_options.cfm' + server_data;
		URL += '&FileSize=' + escape(fileObj.size.toString());
		URL += '&FileType=' + escape(fileObj.type);
		
	
		var OptionsDiv = fileObj.id + '_FileOptions';
		
		wRef.CreateChildFrame(OptionsDiv, null);
		
		AjaxLoadPageToDiv(OptionsDiv, URL);
		
		var ttwRef = p_session.Framework.FindWindow('FileBrowser15');
		ttwRef.RefreshClientArea();
...
FileProgress.prototype.SetComplete = function() {
	...
	var oSelf = this;
	setTimeout(function() { oSelf.Disappear(); }, 1);
};
