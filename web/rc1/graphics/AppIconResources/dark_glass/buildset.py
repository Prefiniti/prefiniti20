...
#
#         USAGE:  python buildiconset.py 'iconsetname-version' (without quotes) 
#
#                 Example: python buildiconset.py Dark_Glass-2.4
#
#                 This will give you a tarball named Dark_Glass-2.4.tar.bz2
...
#       UPDATED:  01/06/2008 11:16:34 PM EST
#       LICENSE:  GPL
#===============================================================================

# Test the Python installation.
try:
    import sys, os, os.path, tarfile, shutil
except:
    print '''
    Is Python installed?\n
...
def scenario1():
    ...
    global source, source1, dirs, dirs1, dirs2, dirSv, size, size1, y, z, subdirs, subdirs1, src, keep
    source = '128x128/'
    source1 = '32x32/'
    dirs = [ '96x96/', '72x72/', '64x64/', '56x56/', '48x48/', '32x32/', '22x22/', '16x16/' ]
    dirs1 = [ '96x96/', '72x72/', '64x64/', '56x56/', '48x48/' ]
    dirs2 = [ '22x22/', '16x16/' ]
    dirSv = dirs2
    size = [ 96, 72, 64, 56, 48, 32, 22, 16 ]
    size1 = [ 22, 16 ]
    list = []
    list1 = []
    ...
    for i in subdirs:
        for j in os.listdir(source+i):
            if j.endswith('.xpm'):
                os.system('convert '+source+i+j+' '+source+i+j.strip('.xpm')+'.png')
                print 'Converted '+source+i+j+' '+'to PNG format.'
    for i in subdirs1:
        for j in os.listdir(source1+i):
            if j.endswith('.xpm'):
                os.system('convert '+source1+i+j+' '+source1+i+j.strip('.xpm')+'.png')
                print 'Converted '+source1+i+j+' '+'to PNG format.'
...
def scenario3():
    ...
    print 'Beginning the resizing process...'
    print ' '
    global source, source1, dirs, dirs1, dirSv, size, size1, y, z, subdirs, subdirs1, src, keep
    source = '128x128/'
    source1 = source
    dirs = [ '96x96/', '72x72/', '64x64/', '56x56/', '48x48/', '32x32/', '22x22/', '16x16/' ]
