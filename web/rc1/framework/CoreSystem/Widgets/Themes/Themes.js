function LoadTheme(ThemeName, ThemeType, Silent) 
{
	...
	if (!Silent) {
		var ws = KGetSetting(AuthenticationRecord.UserID, "PDM:MONITORTYPE")
		var themeWallpaper;
		...
		/*var ab = new AlertBox("You have chosen the " + ThemeName + " desktop theme.<br><br>Changes will take effect next time you reload Prefiniti.", "Desktop Themes", "/graphics/AppIconResources/crystal_project/32x32/actions/colorize.png");
		ab.ShowRestart = true;
		ab.Show();*/
	}
...
function SetDesktopWallpaper(NewWallpaper, DisplayType)
{
	...
	p_session.Framework.PostLocalMessage('PDesktopWindow', IWC_SCREENRESIZED, C_SESSIONMANAGER, sdim);
}